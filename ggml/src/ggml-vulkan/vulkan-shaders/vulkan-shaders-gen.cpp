...
void process_shaders() {
    ...
    for (std::string dim_str : {"", "_3d"}) {
        for (std::string bda : {"32", "64"}) {
            string_to_spv("im2col" + dim_str + "_f32_bda" + bda, "im2col" + dim_str + ".comp", merge_maps(base_dict, {{"A_TYPE", "float"}, {"D_TYPE", "float"}, {"D_SIZE", "4"}, {"BDA_SIZE", bda}}));
            string_to_spv("im2col" + dim_str + "_f32_f16_bda" + bda, "im2col" + dim_str + ".comp", merge_maps(base_dict, {{"A_TYPE", "float"}, {"D_TYPE", "float16_t"}, {"D_SIZE", "2"}, {"BDA_SIZE", bda}}));
            string_to_spv("im2col" + dim_str + "_f32_f16_rte_bda" + bda, "im2col" + dim_str + ".comp", merge_maps(base_dict, {{"A_TYPE", "float"}, {"D_TYPE", "float16_t"}, {"D_SIZE", "2"}, {"RTE16", "1"}, {"BDA_SIZE", bda}}));
        }
    }
