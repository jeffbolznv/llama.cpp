...
static vk_buffer ggml_vk_create_buffer(vk_device& device, size_t size, const std::initializer_list<vk::MemoryPropertyFlags> & req_flags_list) {
    ...
    vk::MemoryAllocateFlagsInfo mem_flags { vk::MemoryAllocateFlagBits::eDeviceAddress };
    ...
    for (auto it = req_flags_list.begin(); it != req_flags_list.end(); it++) {
        ...
        try {
            buf->device_memory = device->device.allocateMemory({ mem_req.size, memory_type_index, &mem_flags });
            break;
        } catch (const vk::SystemError& e) {
            // loop and retry
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        device->shaderInt64 = device_features2.features.shaderInt64;
...
static void ggml_vk_im2col(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst, bool dryrun = false) {
    ...
    vk::DeviceAddress dst_addr = d_buf->bda_addr + vk_tensor_offset(dst) + dst->view_offs;
